...
def satar_iteration(left,right):
    ...
    for item in range(900):
        ...
        if people_select[item] == 0 : #选择左边的门
            ...
            print('当前是 左 人数：', front_people_num_left, front_people_num_right,'距离：',dis_left,dis_right,'坐标',x[item],y[item])

        else: #选择右边的门
            #计算离门小于该人员的距离的总人数
            front_people_num_right = select_right_dis.index(dis_right)
            front_people_num_left = compareDis(dis_left,select_left_dis)
            print('当前是 右 人数：', front_people_num_left, front_people_num_right,'距离：',dis_left,dis_right,'坐标', x[item],y[item])
        ...
        if left_time < right_time :
            if door_left_time < left_time:
                door_left_time = left_time
            #存储改点的距离
            select_left_dis_set.append(dis_left)
            #标记选择的出口
            people_select_set.append(0)
        else:
            if door_right_time < right_time:
                door_right_time = right_time
    ...
    count = count + 1
    print('迭代:',count,'次',len(select_left_dis_set),len(select_right_dis_set),'time:',door_left_time,door_right_time)
    if door_left_time < door_right_time + 30:

        return 0,people_select
    else:
        return count,people_select
...
if __name__ == '__main__':
    ...
    for i in range(900):
        dict[i]=[left_door_distance_set[i],right_door_distance_set[i]]
