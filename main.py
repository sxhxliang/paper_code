...
def calLeftPeoPleTime():
    global dict_select_door_left
    global dict_select_door_left_time
...
#得到选择右门每个人的时间字典
def calRightPeoPleTime(dict):
    global dict_select_door_right_time
    d = sorted(dict.items(), key=lambda x: x[1][0])
    for i in range(len(d)):
        item = d[i]
        people_id = item[0]
...
def iteration_left():
    ...
    leftPeoPleTime = calLeftPeoPleTime(dict_select_door_left)
    print(leftPeoPleTime)
...
def iteration_right():
    ...
    rightPeoPleTime = calRightPeoPleTime(dict_select_door_right)
    time = sorted(rightPeoPleTime.items(),key=lambda x: x[1], reverse=True)
    people_id = time[0][0]
    people_time = time[0][1]

    people_to_left_dis = dict_select_door_right[people_id][0]
